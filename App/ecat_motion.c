 ...
 * PB12+PB13物理按钮(常开+常闭双通道互相校验, 见 estop_pressed())。
 */
#include "soem/soem.h"
 ...
 *
 * ★2026-08 实机反馈: 原1500(≈0.034mm/300ms)在真顶到机械限位后, 哪怕只是编码器
 * 噪声/丝杠间隙回弹/电机继续小幅顶死带来的微小蠕动, 也很容易在300ms内累计超过
 * 这34微米, 计数器被不断清零重来, 导致堵转永远判不成立、vel_cmd全程不降,
 * 一直顶着限位推到 HOME_MAX_FRAMES 强制超时, 超时后再靠 ramp_to_zero() 强制刹车
 * (见其函数头注释, 现已修掉刹车瞬间"往外弹一下"的cmd_pos错位问题, 但迟迟不判停
 * 本身还是要靠松阈值解决)。18000(≈0.41mm/300ms, 折合允许蠕动速度≈1.4mm/s, 仍远
 * 低于收缩速度3.75mm/s, 不会跟正常收缩中的真实位移混淆)是按"给编码器噪声/间隙
 * 蠕动留出十几倍余量"估的默认值, 不是实测——★待实机核对: 如果换了新值后仍然
 * 迟迟不判停(说明真实蠕动比估的还大), 可以再调大; 如果反而在还没真到限位、
 * 平台仍在正常收缩时就提前误判"到位"(比如中途被什么东西轻微卡顿了一下), 说明
 * 调过头了, 要调小。 */
#define STALL_WIN_EPS  18000        /* 一个窗口内累计位移小于此值(pulse)=没在动 */
 ...
 * PB12=常开(NO)触点, PB13=常闭(NC)触点, 两路接线方式完全相同(内部上拉+另一端
 * 接GND, 只是分别接按钮的NO/NC两个触点)。
 *   没按: NO断开→PB12被上拉高电平; NC闭合→PB13被拉到GND低电平。
 *   按下: NO闭合→PB12变低电平; NC断开→PB13被上拉变高电平。
 * 正常情况下两路读数永远"互补"(一高一低); 只要有任一路示"已按下"——包括真按下,
 * 以及只有一路因接触不良/断线而卡在"按下"读数——都触发急停, 不会因为单路接触
 * 故障而悄悄失效(常开单独接线的方案没有这层交叉校验, 断线会被当成"没按")。
 *
 * ⚠ 选型踩过坑: 最早选的是PB0/PB1, 固件代码里确实没人用, 但把原理图(战神开发板)
 * 渲染成图核对后发现PB0=T_SCK、PB1=T_PEN, 物理上早接了板载电阻屏的触摸控制器,
 * 不是空闲脚——单靠"代码没读它"判断空闲不可靠, 这块板子几乎每个GPIO在原理图上
 * 都挂了摄像头/SD卡/NRF无线/CAN/RS485等某个板载外设的自定义网络名。PB12/PB13
 * 是逐个把原理图CPU符号两侧引脚列表过了一遍后确认的两个"网络名一栏空白"的
 * 干净引脚(不属于以上任何外设), 换成这两个才是真正安全的。*/
#define ESTOP_NO_PORT  GPIOB
#define ESTOP_NO_PIN   GPIO_PIN_12
 ...
 * ★ 进来第一件事: 把闭环目标位置cmd_pos对齐到当前实际位置(仅在方向已锁定时,
 *   未锁定走纯前馈本就不受这个问题影响) —— run_homing()收缩过程是裸速度指令,
 *   完全不经过vel_closed(), cmd_pos在归零期间根本不会跟着走; 如果不resync,
 *   直接拿归零前残留的旧cmd_pos(通常≈0, 对应"伸出状态"那个参考系)去算闭环误差,
 *   只要归零已经收缩出去一段距离, 误差就会巨大, 表现为"一进这个函数就突然
 *   往反方向猛冲一截"——这正是归零超时/归零中途急停后"突然往外弹出一点"的根因。
 *   对康复模式/体感模式的正常急停路径, cmd_pos全程本就跟实际同步, 这里resync
 *   是没有副作用的空操作(err本来就接近0)。 */
static void ramp_to_zero(void)
{
   for (int sl = 1; sl <= ctx.slavecount; sl++) {
      if (fb_sign[sl] != 0) {
         double raw = (double)(read_pos63(sl) - start_pos[sl]);
         cmd_pos[sl] = fb_sign[sl] * raw;
      }
      corr_filt[sl] = 0.0;
   }
